# fuzztesting Campaign Features

This document describes the new campaign-based features added to fuzztesting.

## Overview

fuzztesting has evolved from a simple job-based fuzzing orchestrator to a comprehensive campaign-based fuzzing platform. The new features enable better organization, deduplication, and corpus management while maintaining the simplicity that makes fuzztesting unique.
...
1. **Start fuzztesting with Docker Compose:**
   ```bash
   docker compose up -d
   ```
...
The campaign features maintain fuzztesting's philosophy of simplicity:
